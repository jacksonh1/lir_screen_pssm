...
COUNT_MATRICES = dl.get_count_matrices("v1")
TEST_SETS = dl.get_test_sets("v1")
df = TEST_SETS.lir_central_augmented.copy()
...
psi_blast_pssm_dict = {
    "screen_all_binders-psiblast": pd.read_csv(
        env.PROCESSED_DATA_DIR / "v1" / "pssms" / "screen-all_binders_psiblast_pssm.csv"
    ),
    ...
    "ilir_binders-psiblast": pd.read_csv(
        env.PROCESSED_DATA_DIR / "v1" / "pssms" / "fg-ilir_bg-proteome_psiblast.csv"
    ),
}
...
# %%
# fg_sets = [i for i in COUNT_MATRICES.__dict__.keys() if 'file' not in i]
# bg_sets = list(BGFREQS.__dict__.keys())
...
for p in parameter_combinations:
    ...
    if fg_set in psi_blast_pssm_dict:
        ...
        fg = getattr(COUNT_MATRICES, fg_set)
        bg = getattr(BGFREQS, bg_set)
